...
exports.error_handle = async function (next, connection, params) {
  ...
  try {
    ...
    server.notes
      .sendTelegramErrorMessage(new Error(errorMessage), `${this.accountRequest} - ${harakaId} - message_logging`)
      .then()
    next()
  } catch (err) {
    emailLog.error(err)
...
async function updateEmailTxnStatus(emailTxnId, statusCode, statusMessage, extraData = '') {
  ...
  await EmailDeliveryStatus.upsert({
    emailTxnId,
    status: statusCode,
    statusMessage,
    extraData,
